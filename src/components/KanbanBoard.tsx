...
const tasksByColumn$ = (column: string) =>
  queryDb(
    tables.tasks.where({ column }).orderBy('position', 'asc'),
    { label: `tasks-${column}` }
  )
...
export default function KanbanBoard() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (columns.includes(overId as ColumnType)) {
      ...
      const draggedIndex = tasksInColumn.findIndex(t => t.id === draggedTask.id)
    ...
    if (draggedTask.column !== targetColumn || draggedTask.position !== targetPosition) {
  ...
  const handleUpdateTask = (taskId: string, title: string, description: string) => {
    store.commit(events.taskUpdated({ 
      id: taskId, 
      title, 
      description,
      updatedAt: new Date()
        ...
        <div className="modal-overlay" onClick={() => setNewTaskColumn(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button onClick={() => handleAddTask(newTaskColumn)}>Add Task</button>
